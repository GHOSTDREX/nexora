...
Ported from the standalone `Fertilizer Recommendation/src/fertilizer_validator.py`
prototype — logic unchanged, only made relative to this package's own data file.
"""
...
def _training_ranges() -> dict[str, tuple[float, float]]:
    path = Path(__file__).resolve().parent / "data" / "fertilizer_recommendation.csv"
...
def nutrient_status(nitrogen: float, phosphorus: float, potassium: float, soil_ph: float) -> dict[str, str]:
    return {
        "nitrogen": "Low" if nitrogen < 60 else "Moderate" if nitrogen < 100 else "High",
        "phosphorus": "Low" if phosphorus < 40 else "Moderate" if phosphorus < 65 else "High",
        "potassium": "Low" if potassium < 40 else "Moderate" if potassium < 70 else "High",
        "soil_ph": "Acidic" if soil_ph < 5.5 else "Alkaline" if soil_ph > 7.5 else "Suitable Range",
    }
